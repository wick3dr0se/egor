...
/// Constructed via [`crate::graphics::Graphics`]  
/// Primitives are automatically submitted when the builder is dropped  
///
...
/// Batch that accumulates GPU primitives for submission
#[derive(Default)]
    ...
    fn drop(&mut self) {
        ...
        let verts: Vec<Vertex> = points
            ...
            .map(|p| {
                ...
                Vertex::new(world.into(), self.color, [0.0, 0.0])
            })
            .collect();
    ...
    fn drop(&mut self) {
        ...
        let verts: Vec<_> = rect
            ...
            .map(|(&corner, &uv)| {
                ...
                Vertex::new(world.into(), self.color, uv)
            })
            .collect();
